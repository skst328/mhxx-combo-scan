...
通常の検索で見つからないとき、**途中で乱数が余分に進んだ可能性**を調べる。

```
1. 差分列の先頭 18 個だけで位置の候補を探す（走査は 1 回）
2. 各候補から乱数を順に照合し、合わなくなったらずれ幅を探して記録する
3. 最後まで合わせられた候補がちょうど 1 つなら、それを採用する
```

位置さえ決まれば探索は不要で、乱数を数百ステップ進めて突き合わせるだけなので一瞬で終わる。
...
| `PROBE_LEN` | 18 | `search.rs` | 診断で位置を定めるのに使う差分の個数 |

`YIELD_MIN` / `YIELD_MAX` を変えるときは、基準実装 `tools/ref_reader.py` も同時に直す。
...
| | 読み取り | 検索 10<sup>7</sup> |
|---|---:|---:|
| 基準実装 (Python + numba) | 1879 µs/コマ | 0.16 秒 |
| Rust (ネイティブ) | 85 µs/コマ | 0.07 秒 |
| **Rust (wasm)** | **117 µs/コマ** | **0.11 秒** |

同じパソコン 1 台で測った値で、**絶対値は端末によって変わる。**
30 秒の動画（900 コマ）ならこの環境で読み取りは約 0.1 秒だった。
いずれにせよ**全体の所要時間は動画のデコードでほぼ決まる。**
